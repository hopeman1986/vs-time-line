module webglext {
    ...
    export class OrderedSet<V> {
        ...
        hasId( id : string ) : boolean {
            return ( isString( id ) && isNotEmpty( this._valuesMap[ id ] ) );
        }
        ...
        add( value : V, index? : number, moveIfExists? : boolean ) {
            var index = ( isNotEmpty( index ) ? index : this._valuesArray.length );
            if ( !isNotEmpty( moveIfExists ) ) moveIfExists = false;
    ...
    export class OrderedStringSet {
        ...
        hasValue( value : string ) : boolean {
            return ( isString( value ) && isNotEmpty( this._indexes[ value ] ) );
        }
