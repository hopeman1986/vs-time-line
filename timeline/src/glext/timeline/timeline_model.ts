module webglext {
    ...
    export interface TimelineEvent {
        ...
        label : string;
        // the minimum allowed value of start_time
        startLimit_ISO8601? : string;
        // the maximum allowed value of end_time
    ...
    export class TimelineCursorModel {
        ...
        setAttrs( cursor : TimelineCursor ) {
            ...
            this._lineColor = ( isNotEmpty( cursor.lineColor ) ? parseCssColor( cursor.lineColor ) : null );
            this._textColor = ( isNotEmpty( cursor.textColor ) ? parseCssColor( cursor.textColor ) : null );
        ...
        snapshot( ) : TimelineCursor {
            return {
                ...
                lineColor: ( isNotEmpty( this._lineColor ) ? this._lineColor.cssString : null ),
                textColor: ( isNotEmpty( this._textColor ) ? this._textColor.cssString : null ),
    ...
    export class TimelineAnnotationModel {
        ...
        setAttrs( annotation : TimelineAnnotation ) {
            ...
            this._time_PMILLIS = isNotEmpty( annotation.time_ISO8601 ) ? parseTime_PMILLIS( annotation.time_ISO8601 ) : undefined;
    ...
    export class TimelineTrackModel {
        ...
        setAttrs( event : TimelineEvent ) {
            // Don't both checking whether values are going to change -- it's not that important, and it would be obnoxious here
            this._startLimit_PMILLIS = ( isNotEmpty( event.startLimit_ISO8601 ) ? parseTime_PMILLIS( event.startLimit_ISO8601 ) : null );
            this._endLimit_PMILLIS = ( isNotEmpty( event.endLimit_ISO8601 ) ? parseTime_PMILLIS( event.endLimit_ISO8601 ) : null );
            this._start_PMILLIS = parseTime_PMILLIS( event.start_time );
            this._end_PMILLIS = parseTime_PMILLIS( event.end_time );
            this._label = event.label;
            this._labelIcon = event.labelIcon;
            this._userEditable = ( isNotEmpty( event.userEditable ) ? event.userEditable : false );
            ...
            this._fgColor = ( isNotEmpty( event.fgColor ) ? parseCssColor( event.fgColor ) : null );
            this._bgColor = ( isNotEmpty( event.bgColor ) ? parseCssColor( event.bgColor ) : null );
            this._bgSecondaryColor = ( isNotEmpty( event.bgSecondaryColor ) ? parseCssColor( event.bgSecondaryColor ) : null );
            this._borderColor = ( isNotEmpty( event.borderColor ) ? parseCssColor( event.borderColor ) : null );
            this._borderSecondaryColor = ( isNotEmpty( event.borderSecondaryColor ) ? parseCssColor( event.borderSecondaryColor ) : null );
            ...
            this._isBorderDashed = ( isNotEmpty( event.isBorderDashed ) ? event.isBorderDashed : false );
            this._fillPattern = ( isNotEmpty( event.fillPattern ) ? FillPattern[ event.fillPattern ] : FillPattern.solid );
            this._attrsChanged.fire( );
        }
        ...
        setInterval( start_PMILLIS : number, end_PMILLIS : number ) {
            if ( start_PMILLIS !== this._start_PMILLIS || end_PMILLIS !== this._end_PMILLIS ) {
                ...
                var underStartLimit = isNotEmpty( this._startLimit_PMILLIS ) && start_PMILLIS < this._startLimit_PMILLIS;
                var overEndLimit = isNotEmpty( this._endLimit_PMILLIS ) && end_PMILLIS > this._endLimit_PMILLIS;
                ...
                if ( isNotEmpty( this._startLimit_PMILLIS ) && isNotEmpty( this._endLimit_PMILLIS ) && durationLimit_PMILLIS < duration_PMILLIS ) {
        ...
        private limit_start_PMILLIS( start_PMILLIS : number ) {
            return isNotEmpty( this._startLimit_PMILLIS ) ? Math.max( start_PMILLIS, this._startLimit_PMILLIS ) : start_PMILLIS;
        }
        
        private limit_end_PMILLIS( end_PMILLIS : number ) {
            return isNotEmpty( this._endLimit_PMILLIS ) ? Math.min( end_PMILLIS, this._endLimit_PMILLIS ) : end_PMILLIS;
        }
       ...
       snapshot( ) : TimelineEvent {
            return {
                ...
                startLimit_ISO8601: ( isNotEmpty( this._startLimit_PMILLIS ) ? formatTime_ISO8601( this._startLimit_PMILLIS ) : null ),
                endLimit_ISO8601: ( isNotEmpty( this._endLimit_PMILLIS ) ? formatTime_ISO8601( this._endLimit_PMILLIS ) : null ),
                start_time: formatTime_ISO8601( this._start_PMILLIS ),
                end_time: formatTime_ISO8601( this._end_PMILLIS ),
                label: this._label,
                ...
                bgColor: ( isNotEmpty( this._bgColor ) ? this._bgColor.cssString : null ),
                bgSecondaryColor: ( isNotEmpty( this._bgSecondaryColor ) ? this._bgSecondaryColor.cssString : null ),
                fgColor: ( isNotEmpty( this._fgColor ) ? this._fgColor.cssString : null ),
                borderColor: ( isNotEmpty( this._borderColor ) ? this._borderColor.cssString : null ),
                borderSecondaryColor: ( isNotEmpty( this._borderSecondaryColor ) ? this.borderSecondaryColor.cssString : null ),
    ...
    export class TimelineRowModel {
        ...
        constructor( row : TimelineRow ) {
            ...
            var min : number = isNotEmpty( row.yMin ) ? row.yMin : 0;
            var max : number = isNotEmpty( row.yMax ) ? row.yMax : 1;
        ...
        setAttrs( row : TimelineRow ) {
            ...
            this._bgColor = ( isNotEmpty( row.bgColor ) ? parseCssColor( row.bgColor ) : null );
            this._fgLabelColor = ( isNotEmpty( row.fgLabelColor ) ? parseCssColor( row.fgLabelColor ) : null );
            this._bgLabelColor = ( isNotEmpty( row.bgLabelColor ) ? parseCssColor( row.bgLabelColor ) : null );
        ...
        snapshot( ) : TimelineRow {
            return {
                ...
                cursorGuid: this._cursorGuid,
                bgColor: ( isNotEmpty( this._bgColor ) ? this._bgColor.cssString : null ),
                bgLabelColor: ( isNotEmpty( this._bgLabelColor ) ? this._bgLabelColor.cssString : null ),
                fgLabelColor: ( isNotEmpty( this._fgLabelColor ) ? this._fgLabelColor.cssString : null ),
    ...
    export class TimelineGroupModel {
        ...
        snapshot( ) : TimelineGroup {
            return {
                ...
                label: this._label,
                hidden: this._hidden,
                collapsed: ( isNotEmpty( this._collapsed ) ? this._collapsed : false ),
    ...
    export class TimelineRootModel {
        ...
        constructor( root : TimelineRoot ) {
            ...
            this._rowGuids=new OrderedStringSet();
    ...
    export interface TimelineMergeStrategy {
        ...
        updateEventModel( eventModel : TimelineTrackModel, newEvent : TimelineEvent );
    ...
    export class TimelineModel {
        ...
        private _events : OrderedSet<TimelineTrackModel>;
        ...
        constructor( timeline? : Timeline ) {
            ...
            var cursors = ( isNotEmpty( timeline ) && isNotEmpty( timeline.cursors ) ? timeline.cursors : [] );
            ...
            var annotations = ( isNotEmpty( timeline ) && isNotEmpty( timeline.annotations ) ? timeline.annotations : [] );
            ...
            var events = ( isNotEmpty( timeline ) && isNotEmpty( timeline.events ) ? timeline.events : [] );
            this._events = new OrderedSet<TimelineTrackModel>( [], (e)=>e.eventGuid );
            for ( var n = 0; n < events.length; n++ ) {
                this._events.add( new TimelineTrackModel( events[ n ] ) );
            }

            var rows = ( isNotEmpty( timeline ) && isNotEmpty( timeline.rows ) ? timeline.rows : [] );
            this._rows = new OrderedSet<TimelineRowModel>( [], (r)=>r.rowGuid );
            for ( var n = 0; n < rows.length; n++ ) {
                this._rows.add( new TimelineRowModel( rows[ n ] ) );
            }

            var groups = ( isNotEmpty( timeline ) && isNotEmpty( timeline.groups ) ? timeline.groups : [] );
            this._groups = new OrderedSet<TimelineGroupModel>( [], (g)=>g.groupGuid );
            for ( var n = 0; n < groups.length; n++ ) {
                this._groups.add( new TimelineGroupModel( groups[ n ] ) );
            }

            var root = ( isNotEmpty( timeline ) && isNotEmpty( timeline.root ) ? timeline.root : newEmptyTimelineRoot( ) );
        ...
        get events( ) : OrderedSet<TimelineTrackModel> { return this._events; }
        ...
        event( eventGuid : string ) : TimelineTrackModel { return this._events.valueFor( eventGuid ); }
    ...
    export var timelineMergeNewBeforeOld : TimelineMergeStrategy = {
        ...
        updateEventModel: function( eventModel : TimelineTrackModel, newEvent : TimelineEvent ) {
