module webglext {
    ...
    export class TimelineLaneArray {
        ...
        constructor( model : TimelineModel, row : TimelineRowModel, ui : TimelineUi, allowMultipleLanes : boolean ) {
            ...
            function findAvailableLaneNum( event : TimelineTrackModel, startLaneNum : number, endLaneNum : number ) : number {
            ...
            function firstAvailableLaneNum( event : TimelineTrackModel ) : number {
                var laneNum = findAvailableLaneNum( event, 0, self._lanes.length );
                return ( isNotEmpty( laneNum ) ? laneNum : self._lanes.length );
            }

            function addEventToLane( event : TimelineTrackModel, laneNum : number ) {
            ...
            this._addEvent = function( eventGuid : string ) {
                if ( isNotEmpty( self._laneNums[ eventGuid ] ) ) {
                ...
                var updateLaneAssignment = function( ) {
                    ...
                    if ( newEdges_PMILLIS[ 0 ] !== oldEdges_PMILLIS[ 0 ] || newEdges_PMILLIS[ 1 ] !== oldEdges_PMILLIS[ 1 ] ) {
                        ...
                        if ( isNotEmpty( betterLaneNum ) ) {
                            ...
                            if ( !isNotEmpty( newLaneNum ) ) newLaneNum = self._lanes.length;
                            oldLane.remove( event );
        ...
        eventAt( laneNum : number, time_PMILLIS : number ) : TimelineTrackModel {
    ...
    export function effectiveEdges_PMILLIS( ui : TimelineUi, event : TimelineTrackModel ) : number[] {
    ...
    export interface TimelineLane {
        length : number;
        event( index : number ) : TimelineTrackModel;
        isEmpty( ) : boolean;
        eventAtTime( time_PMILLIS : number ) : TimelineTrackModel;
        add( event : TimelineTrackModel );
        remove( event : TimelineTrackModel );
        eventStillFits( event : TimelineTrackModel ) : boolean;
        update( event : TimelineTrackModel );
        collisionsWithInterval( start_PMILLIS : number, end_PMILLIS : number ) : TimelineTrackModel[];
        couldFitEvent( event : TimelineTrackModel ) : boolean;
    }

    // a TimelineLane where no events start/end time overlap
    export class TimelineLaneStack implements TimelineLane {
        private _events : TimelineTrackModel[];
        ...
        add( event : TimelineTrackModel ) {
            ...
            if ( isNotEmpty( this._indices[ eventGuid ] ) ) throw new Error( 'Lane already contains this event: event = ' + formatEvent( event ) );
        ...
        remove( event : TimelineTrackModel ) {
            ...
            if ( !isNotEmpty( i ) ) throw new Error( 'Event not found in this lane: event = ' + formatEvent( event ) );
        ...
        _eventFitsBetween( event : TimelineTrackModel, iBefore : number, iAfter : number ) : boolean {
    ...
    export class TimelineLaneSimple implements TimelineLane {
        ...
        eventAtTime( time_PMILLIS : number ) : TimelineTrackModel {
            ...
            var bestEvent : TimelineTrackModel;
            ...
            for ( var n = this._events.length-1; n >= 0; n-- ) {
                var event : TimelineTrackModel = this._events[n];
        ...
        add( event : TimelineTrackModel ) {
            var eventGuid = event.eventGuid;
            if ( isNotEmpty( this._ids[ eventGuid ] ) ) throw new Error( 'Lane already contains this event: event = ' + formatEvent( event ) );
            
            // for events with undefined order, replace with largest possible negative order so sort is correct
            var order = isNotEmpty( event.order ) ? event.order : Number.NEGATIVE_INFINITY;
        ...
        remove( event : TimelineTrackModel ) {
            var eventGuid = event.eventGuid;
            if ( !isNotEmpty( this._ids[ eventGuid ] ) ) throw new Error( 'Event not found in this lane: event = ' + formatEvent( event ) );
        ...
        _getIndex( queryEvent : TimelineTrackModel ) : number {
            for ( var n = 0; n < this._events.length; n++ ) {
                var event : TimelineTrackModel = this._events[n];
    ...
    function formatEvent( event : TimelineTrackModel ) : string {
        if ( !isNotEmpty( event ) ) {
            return '' + event;
        }
        else {
