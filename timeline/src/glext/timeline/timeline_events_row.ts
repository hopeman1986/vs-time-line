module webglext {
    ...
    export function newEventsRowPaneFactory( eventsRowOpts? : TimelineEventsRowPaneOptions ) : TimelineRowPaneFactory {
        ...
        return function( drawable : Drawable, timeAxis : TimeAxis1D, dataAxis : Axis1D, model : TimelineModel, row : TimelineRowModel, ui : TimelineUi, options : TimelineRowPaneOptions ) : Pane {
            var rowTopPadding      = ( isNotEmpty( eventsRowOpts ) && isNotEmpty( eventsRowOpts.rowTopPadding    ) ? eventsRowOpts.rowTopPadding    : 6 );
            var rowBottomPadding   = ( isNotEmpty( eventsRowOpts ) && isNotEmpty( eventsRowOpts.rowBottomPadding ) ? eventsRowOpts.rowBottomPadding : 6 );
            var laneHeight         = ( isNotEmpty( eventsRowOpts ) && isNotEmpty( eventsRowOpts.laneHeight       ) ? eventsRowOpts.laneHeight       : 33 );
            var painterFactories   = ( isNotEmpty( eventsRowOpts ) && isNotEmpty( eventsRowOpts.painterFactories ) ? eventsRowOpts.painterFactories : [] );
            var allowMultipleLanes = ( isNotEmpty( eventsRowOpts ) && isNotEmpty( eventsRowOpts.allowMultipleLanes ) ? eventsRowOpts.allowMultipleLanes : true );
            ...
            var eventAtCoords = function( viewport : BoundsUnmodifiable, i : number, j : number ) : TimelineTrackModel {
            ...
            var eventAtPointer = function( ev : PointerEvent ) : TimelineTrackModel {
            ...
            var isInsideAnEvent : Mask2D = function( viewport : BoundsUnmodifiable, i : number, j : number ) : boolean {
                return isNotEmpty( eventAtCoords( viewport, i, j ) );
            };
            ...
            var eventDragEvents : TimelineTrackModel[] = [];
            ...
            function allUserEditable( events : TimelineTrackModel[] ) : boolean {
            ...
            function chooseEventDragMode( ui : TimelineUi, mouseTime_PMILLIS : number, eventDragEvents : TimelineTrackModel[] ) : string {
            ...
            var updateCursor = function( ) {
                if ( !eventDragMode ) {
                    ...
                    if ( selection.selectedEvents.length > 1 ) {
                        ...
                        var hoveredEvents = ( isNotEmpty( hoveredEvent ) ? [ hoveredEvent ] : [] );
            ...
            var dragEventCenter = function( ) {
                if ( eventDragMode === 'center' ) {
                    ...
                    for ( var n = 0; n < eventDragEvents.length; n++ ) {
                        ...
                        if ( isNotEmpty( eventStartSnapShift_MILLIS ) ) {
                            if ( !isNotEmpty( snapShift_MILLIS ) || Math.abs( eventStartSnapShift_MILLIS ) < Math.abs( snapShift_MILLIS )) {
                        ...
                        if ( isNotEmpty( eventEndSnapShift_MILLIS ) ) {
                            if ( !isNotEmpty( snapShift_MILLIS ) || Math.abs( eventEndSnapShift_MILLIS ) < Math.abs( snapShift_MILLIS )) {
                                snapShift_MILLIS = eventEndSnapShift_MILLIS;
                            }
                        }
                    }
                    if ( !isNotEmpty( snapShift_MILLIS ) ) {
                        snapShift_MILLIS = 0;
                    }
            ...
            var dragEventStart = function( ) {
                if ( eventDragMode === 'start' ) {
                    ...
                    for ( var n = 0; n < eventDragEvents.length; n++ ) {
                        ...
                        if ( isNotEmpty( eventSnapShift_MILLIS ) ) {
                            if ( !isNotEmpty( snapShift_MILLIS ) || Math.abs( eventSnapShift_MILLIS ) < Math.abs( snapShift_MILLIS )) {
    ...
    function eventLimitsPainterHelper( limitsOpts : TimelineEventLimitsPainterOptions, drawable : Drawable, timeAxis : TimeAxis1D, lanes : TimelineLaneArray, ui : TimelineUi, options : TimelineEventsPainterOptions ) {
        ...
        var lineColor               = ( isNotEmpty( limitsOpts ) && isNotEmpty( limitsOpts.lineColor          ) ? limitsOpts.lineColor        : new Color( 1, 0, 0, 1 ) );
        var lineThickness           = ( isNotEmpty( limitsOpts ) && isNotEmpty( limitsOpts.lineThickness      ) ? limitsOpts.lineThickness    : 2.5 );
        ...
        return {
            ...
            fillEvent: function( laneIndex : number, eventIndex : number, indexXys : number, indexRgbas : number, viewport : BoundsUnmodifiable ) : { indexXys : number; indexRgbas : number } {
                ...
                var event : TimelineTrackModel = lane.event( eventIndex );
                ...
                var xLeft  = isNotEmpty( event.startLimit_PMILLIS ) ? timeAxis.tFrac( event.startLimit_PMILLIS ) : 0;
                var xRight = isNotEmpty( event.endLimit_PMILLIS )   ? timeAxis.tFrac( event.endLimit_PMILLIS )   : 1;
    ...
    export function newEventLimitsPainterFactory( limitOpts? : TimelineEventLimitsPainterOptions ) : TimelineEventsPainterFactory {
        ...
        return function( drawable : Drawable, timeAxis : TimeAxis1D, lanes : TimelineLaneArray, ui : TimelineUi, options : TimelineEventsPainterOptions ) : Painter {
            ...
            return function( gl : WebGLRenderingContext, viewport : BoundsUnmodifiable ) {
                ...
                var selectedEvents : OrderedSet<TimelineTrackModel> = ui.selection.selectedEvents;
                ...
                for ( var l = 0; l < lanes.length; l++ ) {
                    ...
                    for ( var e = 0; e < lane.length; e++ ) {
                        ...
                        if ( selectedEvents.hasId( event.eventGuid ) && ( isNotEmpty( event.startLimit_PMILLIS ) || isNotEmpty( event.endLimit_PMILLIS ) ) ) {
    ...
    function eventStripedBarPainterHelper( barOpts : TimelineEventBarsPainterOptions, drawable : Drawable, timeAxis : TimeAxis1D, lanes : TimelineLaneArray, ui : TimelineUi, options : TimelineEventsPainterOptions ) {
        ...
        var topMargin           = ( isNotEmpty( barOpts ) && isNotEmpty( barOpts.topMargin           ) ? barOpts.topMargin           : 1.2 );
        var bottomMargin        = ( isNotEmpty( barOpts ) && isNotEmpty( barOpts.bottomMargin        ) ? barOpts.bottomMargin        : 1.2 );
        var borderThickness     = ( isNotEmpty( barOpts ) && isNotEmpty( barOpts.borderThickness     ) ? barOpts.borderThickness     : 2 );
        var cornerType          = ( isNotEmpty( barOpts ) && isNotEmpty( barOpts.cornerType          ) ? barOpts.cornerType          : JointType.BEVEL );
        var defaultColor        = ( isNotEmpty( barOpts ) && isNotEmpty( barOpts.defaultColor        ) ? barOpts.defaultColor        : options.timelineFgColor.withAlphaTimes( 0.4 ) );
        var defaultColorSecondary = new Color( 1, 1, 1, 1 );
        var minimumVisibleWidth = ( isNotEmpty( barOpts ) && isNotEmpty( barOpts.minimumVisibleWidth ) ? barOpts.minimumVisibleWidth : 0 );
        
        var stripeWidth         = ( isNotEmpty( barOpts ) && isNotEmpty( barOpts.stripeWidth ) ? barOpts.stripeWidth : 5 );
        var stripeSecondaryWidth= ( isNotEmpty( barOpts ) && isNotEmpty( barOpts.stripeSecondaryWidth ) ? barOpts.stripeSecondaryWidth : 5 );
        var stripeSlant         = ( isNotEmpty( barOpts ) && isNotEmpty( barOpts.stripeSlant ) ? barOpts.stripeSlant : 1 );
        var featherWidth        = ( isNotEmpty( barOpts ) && isNotEmpty( barOpts.featherWidth ) ? barOpts.featherWidth : 2 );
        ...
        var xyFrac_vColor_VERTSHADER = concatLines(
         ...
         var fillPattern_FRAGSHADER = webglext.concatLines(
        ...
        return {
            ...
            fillEvent: function( laneIndex : number, eventIndex : number, indexXys : number, indexRgbas : number, viewport : BoundsUnmodifiable, indexRelativeXys: number, indexFillPattern: number ) : { indexXys : number; indexRgbas : number; indexRelativeXys: number; indexFillPattern: number; } {
                ...
                var _topMargin = isNotEmpty( event.topMargin ) ? event.topMargin : topMargin;
                var _bottomMargin = isNotEmpty( event.bottomMargin ) ? event.bottomMargin : bottomMargin;
    ...
    function eventDashedBorderPainterHelper( barOpts : TimelineEventBarsPainterOptions, drawable : Drawable, timeAxis : TimeAxis1D, lanes : TimelineLaneArray, ui : TimelineUi, options : TimelineEventsPainterOptions ) {
        ...
        var cornerType          = ( isNotEmpty( barOpts ) && isNotEmpty( barOpts.cornerType          ) ? barOpts.cornerType          : JointType.BEVEL );
        var defaultColor        = ( isNotEmpty( barOpts ) && isNotEmpty( barOpts.defaultColor        ) ? barOpts.defaultColor        : options.timelineFgColor.withAlphaTimes( 0.4 ) );
        var defaultBorderColor  = ( isNotEmpty( barOpts ) && isNotEmpty( barOpts.defaultBorderColor  ) ? barOpts.defaultBorderColor  : null );
        var selectedBorderColor = ( isNotEmpty( barOpts ) && isNotEmpty( barOpts.selectedBorderColor ) ? barOpts.selectedBorderColor : options.timelineFgColor );
        var minimumVisibleWidth = ( isNotEmpty( barOpts ) && isNotEmpty( barOpts.minimumVisibleWidth ) ? barOpts.minimumVisibleWidth : 0 );
        var dashLength          = ( isNotEmpty( barOpts ) && isNotEmpty( barOpts.dashLength          ) ? barOpts.dashLength          : 5 );
    ...
    function eventIconsPainterHelper( iconOpts : TimelineEventIconsPainterOptions, drawable : Drawable, timeAxis : TimeAxis1D, lanes : TimelineLaneArray, ui : TimelineUi, options : TimelineEventsPainterOptions ) {
        ...
        var topMargin    = ( isNotEmpty( iconOpts ) && isNotEmpty( iconOpts.topMargin    ) ? iconOpts.topMargin    : 1.2 );
        var bottomMargin = ( isNotEmpty( iconOpts ) && isNotEmpty( iconOpts.bottomMargin ) ? iconOpts.bottomMargin : 1.2 );
        var vAlign       = ( isNotEmpty( iconOpts ) && isNotEmpty( iconOpts.vAlign       ) ? iconOpts.vAlign       : 0.5 );
    ...
    function eventLabelsPainterHelper( labelOpts : TimelineEventLabelOptions, drawable : Drawable, timeAxis : TimeAxis1D, lanes : TimelineLaneArray, ui : TimelineUi, options : TimelineEventsPainterOptions ) {
        ...
        var topMargin       = ( isNotEmpty( labelOpts ) && isNotEmpty( labelOpts.topMargin       ) ? labelOpts.topMargin       : 1.2   );
        var bottomMargin    = ( isNotEmpty( labelOpts ) && isNotEmpty( labelOpts.bottomMargin    ) ? labelOpts.bottomMargin    : 1.2   );
        var leftMargin      = ( isNotEmpty( labelOpts ) && isNotEmpty( labelOpts.leftMargin      ) ? labelOpts.leftMargin      : 4     );
        var rightMargin     = ( isNotEmpty( labelOpts ) && isNotEmpty( labelOpts.rightMargin     ) ? labelOpts.rightMargin     : 4     );
        var vAlign          = ( isNotEmpty( labelOpts ) && isNotEmpty( labelOpts.vAlign          ) ? labelOpts.vAlign          : 0.5   );
        var spacing         = ( isNotEmpty( labelOpts ) && isNotEmpty( labelOpts.spacing         ) ? labelOpts.spacing         : 3     );
        var extendBeyondBar = ( isNotEmpty( labelOpts ) && isNotEmpty( labelOpts.extendBeyondBar ) ? labelOpts.extendBeyondBar : false );
        var textMode        = ( isNotEmpty( labelOpts ) && isNotEmpty( labelOpts.textMode    ) ? labelOpts.textMode    : 'force' );

        // Icon options
        var iconsEnabled     = ( isNotEmpty( labelOpts ) && isNotEmpty( labelOpts.iconsEnabled     ) ? labelOpts.iconsEnabled     : true   );
        var iconsForceWidth  = ( isNotEmpty( labelOpts ) && isNotEmpty( labelOpts.iconsForceWidth  ) ? labelOpts.iconsForceWidth  : 'auto' );
        var iconsForceHeight = ( isNotEmpty( labelOpts ) && isNotEmpty( labelOpts.iconsForceHeight ) ? labelOpts.iconsForceHeight : 'auto' );
        var iconsSizeFactor  = ( isNotEmpty( labelOpts ) && isNotEmpty( labelOpts.iconsSizeFactor  ) ? labelOpts.iconsSizeFactor  : 1      );

        // Text options
        var textEnabled      = ( isNotEmpty( labelOpts ) && isNotEmpty( labelOpts.textEnabled      ) ? labelOpts.textEnabled      : true );
        var textDefaultColor = ( isNotEmpty( labelOpts ) && isNotEmpty( labelOpts.textDefaultColor ) ? labelOpts.textDefaultColor : options.timelineFgColor );
        var textFont         = ( isNotEmpty( labelOpts ) && isNotEmpty( labelOpts.textFont         ) ? labelOpts.textFont         : options.timelineFont );
        ...
        return {
            ...
            paintEvent: function( laneIndex : number, eventIndex : number, gl : WebGLRenderingContext, viewport : BoundsUnmodifiable ) {
                ...
                var labelTopMargin = isNotEmpty( event.labelTopMargin ) ? event.labelTopMargin : topMargin;
                var labelBottomMargin = isNotEmpty( event.labelBottomMargin ) ? event.labelBottomMargin : bottomMargin;
                
                var labelVAlign = isNotEmpty( event.labelVAlign ) ? event.labelVAlign : vAlign;
                var labelVPos = isNotEmpty( event.labelVPos ) ? event.labelVPos : labelVAlign;

                var labelHAlign = isNotEmpty( event.labelHAlign ) ? event.labelHAlign : 0;
                var labelHPos = isNotEmpty( event.labelHPos ) ? event.labelHPos : labelHAlign;
                ...
                if ( !( xEnd <= 0 || xStart > 1 ) ) {
                    ...
                    if ( iconsEnabled && event.labelIcon ) {
                        ...
                        if ( isNotEmpty( iconTexture ) ) {
                            ...
                            var wIconKnown = isNotEmpty( iconWidth );
                            var hIconKnown = isNotEmpty( iconHeight );
