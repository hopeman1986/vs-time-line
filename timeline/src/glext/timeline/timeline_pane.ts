module webglext {
    ...
    export function newTimelinePane( drawable : Drawable, timeAxis : TimeAxis1D, model : TimelineModel, options? : TimelinePaneOptions, ui? : TimelineUi ) : TimelinePane {
        ...
        var rowPaneFactoryChooser  = ( isNotEmpty( options ) && isNotEmpty( options.rowPaneFactoryChooser ) ? options.rowPaneFactoryChooser : rowPaneFactoryChooser_DEFAULT );
    
        // Scroll
        var showScrollbar          = ( isNotEmpty( options ) && isNotEmpty( options.showScrollbar ) ? options.showScrollbar : true );
        var scrollbarOptions       = ( isNotEmpty( options ) ? options.scrollbarOptions : null );
        

        // Colors
        var fgColor                     = ( isNotEmpty( options ) && isNotEmpty( options.fgColor                     ) ? options.fgColor                     : white                      );
        var bgColor                     = ( isNotEmpty( options ) && isNotEmpty( options.bgColor                     ) ? options.bgColor                     : rgb( 0.098, 0.165, 0.243 ) );
        var rowLabelColor               = ( isNotEmpty( options ) && isNotEmpty( options.rowLabelColor               ) ? options.rowLabelColor               : fgColor                    );
        var rowLabelBgColor             = ( isNotEmpty( options ) && isNotEmpty( options.rowLabelBgColor             ) ? options.rowLabelBgColor             : bgColor                    );
        var groupLabelColor             = ( isNotEmpty( options ) && isNotEmpty( options.groupLabelColor             ) ? options.groupLabelColor             : fgColor                    );
        var axisLabelColor              = ( isNotEmpty( options ) && isNotEmpty( options.axisLabelColor              ) ? options.axisLabelColor              : fgColor                    );
        var rowBgColor                  = ( isNotEmpty( options ) && isNotEmpty( options.rowBgColor                  ) ? options.rowBgColor                  : rgb( 0.020, 0.086, 0.165 ) );
        var rowAltBgColor               = ( isNotEmpty( options ) && isNotEmpty( options.rowAltBgColor               ) ? options.rowAltBgColor               : rgb( 0.020, 0.086, 0.165 ) );
        var gridColor                   = ( isNotEmpty( options ) && isNotEmpty( options.gridColor                   ) ? options.gridColor                   : gray( 0.5 )                );
        var selectedIntervalFillColor   = ( isNotEmpty( options ) && isNotEmpty( options.selectedIntervalFillColor   ) ? options.selectedIntervalFillColor   : rgba( 0, 0.6, 0.8, 0.157 ) );
        var selectedIntervalBorderColor = ( isNotEmpty( options ) && isNotEmpty( options.selectedIntervalBorderColor ) ? options.selectedIntervalBorderColor : rgb( 0, 0.2, 1.0 )         );

        // Axes
        var showTopAxis      = ( isNotEmpty( options ) && isNotEmpty( options.showTopAxis    ) ? options.showTopAxis    : true    );
        var showBottomAxis   = ( isNotEmpty( options ) && isNotEmpty( options.showBottomAxis ) ? options.showBottomAxis : true    );
        var topTimeZone      = ( isNotEmpty( options ) && isNotEmpty( options.topTimeZone    ) ? options.topTimeZone    : '+0000' );
        var bottomTimeZone   = ( isNotEmpty( options ) && isNotEmpty( options.bottomTimeZone ) ? options.bottomTimeZone : '+0000' );
        var tickSpacings      = ( isNotEmpty( options ) && isNotEmpty( options.tickSpacings    ) ? options.tickSpacings    : 60      );
        var axisLabelAlign   = ( isNotEmpty( options ) && isNotEmpty( options.axisLabelAlign ) ? options.axisLabelAlign : 0.5      );

        showTopAxis = true;
        topTimeZone = "2123123";
        // showBottomAxis = true;
        // Sizing
        var groupLabelInsets   = ( isNotEmpty( options ) && isNotEmpty( options.groupLabelInsets   ) ? options.groupLabelInsets   : newInsets( 6, 10 ) );
        var rowLabelInsets     = ( isNotEmpty( options ) && isNotEmpty( options.rowLabelInsets     ) ? options.rowLabelInsets     : newInsets( 0, 35 ) );
        var rowLabelPaneWidth  = ( isNotEmpty( options ) && isNotEmpty( options.rowLabelPaneWidth  ) ? options.rowLabelPaneWidth  : 140 );
        var rowSeparatorHeight = ( isNotEmpty( options ) && isNotEmpty( options.rowSeparatorHeight ) ? options.rowSeparatorHeight : 2   );
        var scrollbarWidth     = ( isNotEmpty( options ) && isNotEmpty( options.scrollbarWidth     ) ? options.scrollbarWidth     : 16  );
        scrollbarWidth         = showScrollbar ? scrollbarWidth : 0; // if the scrollbar is not showing, set its width to 0
        var axisPaneHeight     = ( isNotEmpty( options ) && isNotEmpty( options.axisPaneHeight     ) ? options.axisPaneHeight     : 40  );
        var draggableEdgeWidth = ( isNotEmpty( options ) && isNotEmpty( options.draggableEdgeWidth ) ? options.draggableEdgeWidth : 6   );
        var snapToDistance     = ( isNotEmpty( options ) && isNotEmpty( options.snapToDistance     ) ? options.snapToDistance     : 10  );
        ...
        var selectedIntervalMode                   = ( isNotEmpty( options ) && isNotEmpty( options.selectedIntervalMode                ) ? options.selectedIntervalMode                : 'range' );
        var centerSelectedIntervalOnDoubleClick    = ( isNotEmpty( options ) && isNotEmpty( options.centerSelectedIntervalOnDoubleClick ) ? options.centerSelectedIntervalOnDoubleClick : true    );
        var defaultMouseWheelListener = function( ev : PointerEvent ) {
            var zoomFactor = Math.pow( axisZoomStep, ev.wheelSteps );
            timeAxis.tZoom( zoomFactor, timeAxis.vAtFrac( xFrac( ev ) ) );
        }
        var mouseWheelListener                     = ( isNotEmpty( options ) && isNotEmpty( options.mouseWheelListener                  ) ? options.mouseWheelListener : defaultMouseWheelListener);
    ...
    function attachTimeAxisMouseListeners( pane : Pane, axis : TimeAxis1D, args : TimelineContentPaneArguments ) {
        ...
        pane.mouseMove.on( function( ev : PointerEvent ) {
            if ( isLeftMouseDown( ev.mouseEvent ) && isNotEmpty( vGrab ) ) {
                axis.tPan( vGrab - axis.vAtFrac( xFrac( ev ) ));
            }
        } );
    ...
    function newTimelineSingleSelectionPainter( timeAxis : TimeAxis1D, interval : TimeIntervalModel, borderColor : Color, fillColor : Color ) : Painter {
        ...
        return function( gl : WebGLRenderingContext, viewport : BoundsUnmodifiable ) {
            if ( isNotEmpty( interval.cursor_PMILLIS  ) ) {
    ...
    function newTimelineRangeSelectionPainter( timeAxis : TimeAxis1D, interval : TimeIntervalModel, borderColor : Color, fillColor : Color ) : Painter {
        ...
        return function( gl : WebGLRenderingContext, viewport : BoundsUnmodifiable ) {
            if ( isNotEmpty( interval.start_PMILLIS ) && isNotEmpty( interval.end_PMILLIS ) ) {
    ...
    function newTimelineContentPane( args : TimelineContentPaneArguments ) : Pane {
        ...
        var attachGroupAttrsChangedListener = function( groupGuid : string, groupIndex : number ) {
            ...
            var groupAttrsChangedListener = function( ) {
                if ( isNotEmpty( group.hidden ) && isNotEmpty( groupContentPanes[groupGuid] ) ) {
    ...
    function newRowBackgroundPainter( args : TimelineContentPaneArguments, guidList : OrderedStringSet, row : TimelineRowModel ) {
        return function( gl : WebGLRenderingContext ) {
            var color = isNotEmpty( row.bgColor ) ? row.bgColor : ( guidList.indexOf( row.rowGuid ) % 2 ? args.options.rowBgColor : args.options.rowAltBgColor );
    ...
    function setupRowContainerPane( args : TimelineContentPaneArguments, parentPane : Pane, guidList : OrderedStringSet, isMaximized : boolean, keyPrefix : string ) {
        ...
        var addRow = function( rowGuid : string, rowIndex : number ) {
            ...
            var rowLabelColorBg : Color = isNotEmpty( row.bgLabelColor ) ? row.bgLabelColor : options.rowLabelBgColor;
            var rowLabelColorFg : Color = isNotEmpty( row.fgLabelColor ) ? row.fgLabelColor : options.rowLabelColor;
            var rowLabelFont : string = isNotEmpty( row.labelFont ) ? row.labelFont : options.font;
            ...
            var rowAttrsChanged = function( ) {
                ...
                rowLabel.fgColor = isNotEmpty( row.fgLabelColor ) ? row.fgLabelColor : options.rowLabelColor;
                rowLabel.font = isNotEmpty( row.labelFont ) ? row.labelFont : options.font;
                rowLabelBackground.color = isNotEmpty( row.bgLabelColor ) ? row.bgLabelColor : options.bgColor;
                drawable.redraw( );
            }
        ...
        var attachAttrsChangedListener = function( rowGuid : string, rowIndex : number ) {
            ...
            var attrsChangedListener = function( ) {
                if ( isNotEmpty( row.hidden && isNotEmpty( rowPanes[rowGuid] ) ) ) {
